 ...
 * <p>
 * GitHub user @maritaria has made some performance improvements which can be found in the
 * comment section of this Gist.
 */
public class GraphPanel extends JPanel {
	...
	private final int NUMBER_Y_DIVISIONS = 10;
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < NUMBER_Y_DIVISIONS + 1; i++) {
			...
			int y0 = getHeight()
					- ((i * (getHeight() - padding * 2 - labelPadding)) / NUMBER_Y_DIVISIONS + padding + labelPadding);
			int y1 = y0;
			if (scores.size() > 0) {
				...
				String yLabel = ((int) ((getMinScore()
						+ (getMaxScore() - getMinScore()) * ((i * 1.0) / NUMBER_Y_DIVISIONS)) * 100)) / 100.0 + "";
	...
	public void setScores(List<Double> scores) {
		...
		revalidate();
		this.repaint();
		System.out.println("[System Operation] Redrawing graph.");
	}
